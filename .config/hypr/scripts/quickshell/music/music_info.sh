...
raw=$(playerctl metadata --format \
    "{{status}}${SEP}{{xesam:title}}${SEP}{{xesam:artist}}${SEP}{{mpris:artUrl}}${SEP}{{mpris:length}}${SEP}{{position}}${SEP}{{playerName}}" \
    2>/dev/null)

if [ -z "$raw" ]; then
    STATUS="Stopped"
else
    IFS=$'\x1f' read -r STATUS TITLE ARTIST rawUrl len_micro pos_micro player_raw <<< "$raw"
fi
