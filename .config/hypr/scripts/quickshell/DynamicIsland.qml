...
PanelWindow {
    ...
    // Weather: refreshes cache then reads real-time temp via --live
    Process {
        id: weatherProc
        command: ["bash", "-c",
            "~/.config/hypr/scripts/quickshell/calendar/weather.sh --json >/dev/null 2>&1; " +
            "~/.config/hypr/scripts/quickshell/calendar/weather.sh --live 2>/dev/null"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    Process {
        id: pageWatcher; running: true
        command: ["bash", "-c",
            "inotifywait -qq -e close_write,moved_to --include 'qs_island_page$' /tmp/ 2>/dev/null; " +
            "if [ -f /tmp/qs_island_page ]; then cat /tmp/qs_island_page; rm -f /tmp/qs_island_page; fi"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                let page = this.text.trim();
                if (page !== "") {
                    if (islandWindow.expanded && islandWindow.currentPage === page) {
                        islandWindow.expanded = false;
                    } else {
                        islandWindow.currentPage = page;
                        islandWindow.expanded = true;
                    }
                }
                pageWatcher.running = false;
                pageWatcher.running = true;
            }
        }
    }
}
